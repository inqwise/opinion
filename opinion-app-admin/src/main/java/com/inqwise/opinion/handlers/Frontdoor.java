...
public class Frontdoor extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(null == referer || !(null == ApplicationConfiguration.Opinion.getSessionDomain() || StringUtils.contains(referer, ApplicationConfiguration.BackOffice.getSessionDomain()))){
		...
		logger.debug("Frontdoor: redirectUrl : '%s'", redirectUrl);
	...
	private String Login(HttpServletRequest request, HttpServletResponse response, URL applicationURL, String returnUrl)
		    ...
		    if (cookies != null) {
		    	for (int i = 0; i < cookies.length; i++) {
		            ...
		            if(cookie.getMaxAge() != 0){
			            ...
			            if (cookieName.equalsIgnoreCase(DataPostmasterDescryptorBase.SESSION_ID_COOKIE_KEY)){
			           		if(null == sid) {
			            		sid = cookie.getValue();
			           		} else {
			            ...
			            if (cookieName.equalsIgnoreCase(DataPostmasterDescryptorBase.UNIQUE_ID_COOKIE_KEY)){
			            	if(null == uid) {
			            		uid = cookie.getValue();
			            	} else {
		...
		if(null == redirectUrl){
		    if(null != sid && !sid.equals("null") && null != uid && !uid.equals("null")) {
		    	...
		    	logger.error("login: One of the cookie keys not saved. sid:'%s', uid:'%s'", sid, uid);
		    	redirectUrl = request.getContextPath() + "/500.html";
