        ...
        dataTable: function (o) {
            ...
            var V = function (p) {
                ...
                for (var i = e; i < u.length; i++) {
                    if (h < options.pagingStart) {
                        var r = $("<tr " + (options.selectable ? "class=\"" + B + "-row-" + i + "\"" : "") + " />").appendTo(Q);
            ...
            if (options.tableColumns != null) {
                ...
                for (var i = 0; i < options.tableColumns.length; i++) {
                    var h = $("<th " + ((i == options.tableColumns.length - 1) ? "class=\"last-item\"" : "") + ((i == 0) ? (options.selectable ? "" : (options.expandable ? "" : "class=\"first-item\"")) : "") + "><div><span>" + options.tableColumns[i].label + "</span></div></th>").appendTo(Y);
                    if (options.tableColumns[i].width != undefined) {
                        h.css({
                            'width': options.tableColumns[i].width
                        })
                    }
                    if (options.tableColumns[i].dir != undefined) {
                        h.css({
                            'direction': options.tableColumns[i].dir
                        })
                    }
