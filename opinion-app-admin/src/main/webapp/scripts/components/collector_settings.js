...
(function($) {
	collectorSettings = function(givenOptions) {
		...
		var handleElements = function() {
			...
			*/
			...
			new validator({
				...
				submitElement : $('#button_save_changes'),
				messages : null,
				accept : function() {
					...
					updateCollector({
						...
						name : domElements.label_CollectorName.text(),
						closeMessage : $('#textarea_close_default_message').val(),
						closeAfterQuotaReached : (enableRestrictions ? domElements.checkbox_CloseAfterAQuotaReached.is(':checked') : undefined), 
						quotaReached : (enableRestrictions ? domElements.input_QuotaReached.val() : undefined),
						closeAfterCertainDate : (enableRestrictions ? domElements.checkbox_CloseAfterACertainDate.is(':checked') : undefined),
						certainDate : (enableRestrictions ? ((domElements.checkbox_CloseAfterACertainDate.is(':checked')) ? getCertainDate() : null) : undefined),
						referer : (enableRestrictions ? ($('#checkbox_allow_access_from_specific_referral_only').is(':checked') ? $.trim($('#text_referer').val()) : "") : undefined),
						isPasswordRequired : (enableRestrictions ? domElements.checkbox_PasswordProtection.is(':checked') : undefined),
						hidePassword : (enableRestrictions ? domElements.checkbox_HidePassword.is(':checked') : undefined),
						password : (enableRestrictions ? ((domElements.checkbox_HidePassword.is(':checked')) ? domElements.input_FakePasswordProtection.val() : domElements.input_PasswordProtection.val()) : undefined),
						allowMultipleResponses : ((parseInt(domElements.dropdown_MultipleResponses[0].value) == 1) ? true : false),
						enablePrevious : getEnablePrevious(),
						enableRssUpdates : domElements.checkbox_EnableRssUpdates.is(':checked'),
						enableEmailNotification : domElements.checkbox_EnableEmailNotification.is(':checked'),
		...
		var getEnablePrevious = function() {
			...
			var radio = $('#container_edit_responses').find('input:radio'); // need to change
