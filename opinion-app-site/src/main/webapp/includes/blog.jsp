...
<%@ page import="com.inqwise.opinion.cms.common.ITag" %>
<%@ page import="com.inqwise.opinion.cms.common.ICategory" %>
<%@ page import="com.inqwise.opinion.cms.managers.TagsManager" %>
...
<%@page import="com.inqwise.opinion.cms.managers.CategoriesManager"%>
...
<%
	...
	String categoryName = StringUtils.trimToNull(request.getParameter("category_name"));
	...
	archiveDate.setTime(urlArchiveFormatter.parse(strArchiveDate));
...
<%
	OperationResult<List<IPost>> postsResult = BlogManager.searchPosts(tagName, categoryName, archiveTotalMonth, searchWord, 100, Blogs.OfficialInqwiseBlog);
...
<%
	if(!postsResult.hasError()){
	postsResult = BlogManager.getPosts(10, Blogs.OfficialInqwiseBlog);
	List<IPost> posts = postsResult.getValue();
	for(IPost post : posts){
%>
				<li><a href="<%=absoluteURL + "/blog/"+ urlArchiveFullFormatter.format(post.getPostDate()) +"/" + post.getUrlTitle()%>" title="<%=post.getTitle()%>" class="arrow-grey"><%=post.getTitle()%></a></li>
<%
	}
}
%>	
...
if(!tagsResult.hasError()){
	...
	for(ITag tag : tags){
%>
			<li class="tag-<%=tag.getGradeByFive()%>"><a href="<%=absoluteURL%>/blog/tags/<%=URLEncoder.encode(tag.getName(), "UTF-8")%>" title="<%=tag.getName()%>"><%=tag.getName()%></a></li>
<%
	}
}
%>
