...
if(!faqsResults.hasError()){
	...
	if(null != searchWord) {
		%>
		Your search for <b style="color: #333">"<%=searchWord %>"</b> returned no matches.
		<%
	}
}
%>
	...
	<div class="wrapper-content-middle">
		...
		<div>
			<ul class="ll">
<%
OperationResult<List<IFaq>> faqsResultsTop = FaqsManager.searchFaqs(null, null, 10);
if(!faqsResultsTop.hasError()){
	List<IFaq> faqs = faqsResultsTop.getValue();
	for(IFaq faq : faqs){
%>
				<li><a href="<%=absoluteURL %>/support/search/<%=URLEncoder.encode(faq.getQuestion()) %>" title="<%=faq.getQuestion() %>" class="arrow-grey"><%=faq.getQuestion() %></a></li>
<%
	}
}
%>
...
var validateSearch = function(){
	...
	if(q.length>0) {
		location.href="<%=absoluteURL %>/support/search/" + escape(q);
	}
};
</script>