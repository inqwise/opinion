...
<h1>
	<%
		if(null != strArchiveDate) {
		String pageTitle = archiveFormatter.format(urlArchiveFormatter.parse(strArchiveDate)) + " Posts";
		p.setWindowTitle(pageTitle);
	%>
				<%=pageTitle%> 
			<%
 				}
...
<%
	OperationResult<List<IPost>> archiveResult = BlogManager.searchPosts(null, null, archiveTotalMonth, null, 100, Blogs.OfficialInqwiseBlog);
	if(!archiveResult.hasError()){
		List<IPost> posts = archiveResult.getValue();
