...
(function($) {
	collectorSettings = function(givenOptions) {
		...
		var loadDays = function() {
			...
			var month = parseInt($("#dropdown_close_after_certain_months").val());
			var year = parseInt($("#dropdown_close_after_certain_years").val());
		...
		var loadMonths = function() {
			...
			for(var x = 0; x < messages.monthNames.length; x++) {
				var optionElem = new Option(messages.monthNames[x],  x);
		...
		var getCertainDate = function() {
			...
			var year = parseInt($("#dropdown_close_after_certain_years").val());
			var month = parseInt($("#dropdown_close_after_certain_months").val());
			var day = parseInt($("#dropdown_close_after_certain_days").val());
			var hours = parseInt($("#dropdown_close_after_certain_hours").val());
			var minutes = parseInt($("#dropdown_close_after_certain_minutes").val());
		...
		var getEnablePrevious = function() {
			...
			var radio = domElements.container_EditResponses.find('input:radio');
		...
		*/
		...
		var getResultsTypeId = function() {
			var resultsTypeId = 1;
			$("input:radio[name=results]").each(function(i, el) {
				if($(el).is(':checked')) {
					resultsTypeId = $(el).attr("value");
				}
			});
			return resultsTypeId;
		};
		...
		var init = function() {
			...
			if(data.closeAfterCertainDate) {
				...
				$("#checkbox_close_after_a_certain_date").prop("checked", true).change();			
			}

			if(data.closeAfterQuotaReached) {
				quotaReached = data.quotaReached;
				$("#checkbox_close_after_a_quota_reached").prop("checked", true).change();
			}
			...
			$("input:radio[name=results][value=" + data.resultsTypeId + "]").prop('checked', true);
			...
			if(data.closeMessage != undefined) {
				if(data.closeMessage != "") {
					...
					$('#textarea_close_default_message').val(messages.defaultCloseMessage);
				}
			} else {
				$('#textarea_close_default_message').val(messages.defaultCloseMessage);
			}
			...
			new validator({
				...
				accept : function() {
					...
					updateCollector({
						...
						closeAfterQuotaReached : $("#checkbox_close_after_a_quota_reached").is(":checked"), 
						quotaReached : $("#input_quota_reached").val(),
						closeAfterCertainDate : $("#checkbox_close_after_a_certain_date").is(":checked"),
						certainDate : (($("#checkbox_close_after_a_certain_date").is(":checked")) ? getCertainDate() : null),
						allowMultipleResponses : (($("#dropdown_multiple_responses").val() == 1) ? true : false),
						/* enablePrevious : getEnablePrevious(), */
						enableRssUpdates : false,
						enableEmailNotification : $("#checkbox_enable_email_notification").is(":checked"),
						enableSSLEncription : false,
						resultsTypeId : getResultsTypeId(),
						success : function() {
