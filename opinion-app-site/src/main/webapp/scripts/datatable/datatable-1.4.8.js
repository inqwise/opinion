(function ($) {
    $.fn.extend({
        dataTable: function (o) {
            ...
            var B = A.attr('id');
            A.addClass('data-table').append("<div class=\"data-table-header\" id=\"" + B + "_header\">" + "<div class=\"" + (options.selectable ? "container-actions-selectable" : "container-actions")  + " \" id=\"" + B + "_container_actions\"></div><div class=\"container-filters\" id=\"" + B + "_container_filters\"></div>" + "</div>" + "<div class=\"data-table-body\">" + "<table id=\"" + B + "_table_view\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"></table>" + "</div>" + "<div class=\"data-table-footer\">" + "<div class=\"container-paging\" id=\"" + B + "_container_paging\">" + "<a href=\"javascript:;\" id=\"" + B + "_button_previous\" class=\"data-table-button-previous btn btn-default button-white\" title=\"Previous\"><span><i class=\"icon-previous\">&nbsp;</i></span></a><a href=\"javascript:;\" id=\"" + B + "_button_next\" class=\"data-table-button-next btn btn-default button-white\" title=\"Next\"><span><i class=\"icon-next\">&nbsp;</i></span></a>" + "</div>" + "<div class=\"container-results\" id=\"" + B + "_container_results\">" + "<div class=\"cell\">" + "<div id=\"" + B + "_label_results\"></div>" + "</div>" + "</div>" + "<div class=\"container-show-rows\" id=\"" + B + "_container_show_rows\">" + "<div class=\"cell\"><span class=\"label-field\">" + options.messages.showRows + "</span></div>" + "<div class=\"cell select-field\">" + "<select id=\"" + B + "_select_show_rows\"></select>" + "</div>" + "</div>" + "<div class=\"container-goto\" id=\"" + B + "_container_goto\">" + "<div class=\"cell button-field\">" + "<a id=\"" + B + "_button_goto_page\" title=\"Go\" class=\"button-white\"><span>Go</span></a>" + "</div>" + "<div class=\"cell\">" + "<input id=\"" + B + "_input_goto_page\" type=\"text\" autocomplete=\"off\" class=\"goto\" />" + "</div>" + "<div class=\"cell\"><span class=\"label-field\">Go to:</span></div>" + "</div>" + "</div>");
            var C = {};
            ...
            var V = function (p) {
                    ...
                    if (h < options.pagingStart) {
                        ...
                        if(options.groupBy != null) {
                            ...
                            for (var x = 0; x < options.tableColumns.length; x++) {
                                ...
                                if(sort != null) {
                                    if(sort.key == options.tableColumns[x].key) {
                                        //console.log("sortable -> " + options.tableColumns[x].key);
                                        c.addClass("sortable");
                                    }
                                }
                ...
                A.find("#" + B + "_label_results").html("<b>" + (e + 1) + "</b> - <b>" + ((e + options.pagingStart) > j ? j : (e + options.pagingStart)) + "</b> " + options.messages.of + " <b>" + j + "</b>");
                m.val(l + 1)
            };
            ...
            var retrive = function(p) {
                ...
                if (total > 0) {
                    ...
                    A.find("#" + B + "_label_results").html("<b>0</b> - <b>0</b> " + options.messages.of + " <b>0</b>");
                    //F.hide();
                    //H.hide();
                    I.hide();
                    J.hide()
                                ...
                                for(var _y = 0; _y < options.totals[_i].length; _y++) {
                                    if(options.tableColumns[x].key == options.totals[_i][_y].key) {
                                        if(options.totals[_i][_y].formatter != undefined
                                            ...
                                            c.html(options.totals[_i][_y].formatter());
                                        } else {
                                            c.html(options.totals[_i][_y].key);
                                        }
                                    }
                                }
                ...
                var bb = function () {
                    if (options.actions.length > 0) {
                        for (var t = 0; t < options.actions.length; t++) {
                            ...
                            var isSplitButton = options.actions[t].fire != undefined && options.actions[t].actions != undefined ? true : false;
                            var buttonContainer = $("<div class=\"data-table-inline-button-container\"></div>");
                            var buttonGroup = $("<div class=\"btn-group\"></div>");
                            if(isSplitButton) {
                                buttonGroup.appendTo(buttonContainer);
                            }

                            //var c = $("<a href=\"javascript:;\" action=\"" + t + "\" title=\"" + (options.actions[t].label ? options.actions[t].label : "Action" + t) + "\" class=\"btn btn-" + (options.actions[t].color != undefined ? options.actions[t].color : "default") + "\"><span>" + (options.actions[t].icon ? "<i class=\"" + options.actions[t].icon + "\"></i> " : "") + (options.actions[t].label ? options.actions[t].label : "Action" + t) + ((options.actions[t].actions != undefined || options.actions[t].pop != undefined) ? " <i class=\"fa fa-caret-down\"></i>" : "") +"</span></a>").click(function () {

                            var splitButton = null;
                            if(isSplitButton) {
                                splitButton = $("<button action=\"" + t + "\" class=\"btn btn-" + (options.actions[t].color != undefined ? options.actions[t].color : "default") + "\">" + (options.actions[t].icon ? "<i class=\"" + options.actions[t].icon + "\"></i> " : "") + (options.actions[t].label ? options.actions[t].label : "Action" + t) + "</button>").click(function () {
                                    var o = $(this);
                                    var b = o.attr('action');
                                ...
                                }).appendTo(isSplitButton ? buttonGroup : buttonContainer);

                            }

                            var c = $("<button action=\"" + t + "\" class=\"btn btn-" + (options.actions[t].color != undefined ? options.actions[t].color : "default") + " " + (isSplitButton ? "dropdown-toggle" : "") + "\">" + (isSplitButton ? "" : (options.actions[t].icon ? "<i class=\"" + options.actions[t].icon + "\"></i> " : "") + (options.actions[t].label ? options.actions[t].label : "Action" + t)) + ((options.actions[t].actions != undefined || options.actions[t].pop != undefined) ? " <i class=\"fa fa-caret-down\"></i>" : "") + "</button>").click(function () {
                                var o = $(this);
                                var b = o.attr('action');
                                    ...
                                    if(!o.hasClass("ui-button-disabled")) {
                                        ...
                                        if(!o.hasClass('ui-button-selected')) {
                                            if(lastCombo != null && lastCombo != o) {
                                                lastCombo.removeClass('ui-button-selected').removeClass('ui-button-active').closest('.ui-button-dropdown').find('.ui-combo-popup').removeClass('active');
                                            }
                                            lastCombo = o;
                                            o.addClass('ui-button-selected').addClass('ui-button-active').closest('.ui-button-dropdown').find('.ui-combo-popup').addClass('active');
                                        } else {
                                            lastCombo.removeClass('ui-button-selected').removeClass('ui-button-active').closest('.ui-button-dropdown').find('.ui-combo-popup').removeClass('active');
                                        }
                            ...
                            if(options.actions[t].disabled != undefined) {
                                if(options.actions[t].disabled) {
                                    ...
                                    if(isSplitButton) {
                                        splitButton.addClass('ui-button-disabled');
                        ...
                        $('.ui-button-dropdown button.btn, .ui-combo-popup').click(function() { return false; });
