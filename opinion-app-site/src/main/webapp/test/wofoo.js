...
(function () {
    var WufooFieldLogic = Class.create({
        ...
        ratingToggle: function (columnId) {
            if (document.observe) {
                this.setRatingNew();
            } else {
                this.setRatingOld();
        ...
        setRatingNew: function () {
            var self = this;
            document.observe("click", function (e) {
                if (e.findElement(".rating")) {
                    var el = e.findElement(".rating"),
                        inputEl = el.getElementsByTagName("input")[0],
                        currentRating = inputEl.getAttribute("value") || 0,
                        newRating = e.target.getAttribute("value"),
                        elems = el.getElementsByTagName("span");
                    self.toggleRatingElement(inputEl, newRating, currentRating, elems, "__MNG");
                    e.preventDefault();
                }
            });
        },
        setRatingOld: function () {
            var self = this;
            $$(".rating").each(function (el) {
                ...
                el.observe("click", function (event) {
                    ...
                    var currentRating = inputEl.getAttribute("value"),
                        newRating = $this.getAttribute("value");
                    self.toggleRatingElement(inputEl, newRating, currentRating, elems, "__PF");
                    return false;
                });
            });
        },
        toggleRatingElement: function (inputEl, newRating, currentRating, elems, loc) {
            if (newRating == currentRating) {
                newRating = 0;
            }
            for (var i = 0, len = elems.length; i < len; i++) {
                if (elems[i].getAttribute("value") <= newRating) {
                    elems[i].addClassName("clicked");
                } else {
                    elems[i].removeClassName("clicked");
                }
            }
            if (newRating > 0) {
                inputEl.setAttribute("value", newRating);
                this.attachRatingRulesHandler(loc, inputEl);
            } else {
        ...
        checkFileSize: function (evt) {
            ...
            function onChangeHandler(el) {
                return function (evt) {
                    ...
                    $parentLi.removeClassName("error");
                    var errorMessages = $parentLi.getElementsByClassName("error");
                    ...
                    if (file && file !== undefined) {
                        ...
                        if (size > 10) {
                            ...
                            $(clone).observe("change", onChangeHandler(clone));
            ...
            $$(".file").each(function (el) {
                el.observe("change", onChangeHandler(el))
            });
        },
        attachRatingRulesHandler: function (loc, el) {
            if (loc == "__PF") {
                __PF.handleInput(el);
            } else {
                __MNG.handleInput(el);
            };
        }
    });
...
var WufooFormLogic = Class.create({
    ...
    observeFormSubmit: function () {
        ...
        var self = this;
        $(activeForm).observe('submit', function () {
...
var WufooConditions = Class.create({
    ...
    getPhoneInputValue: function (columnId) {
        var phone = $('Field' + columnId).value +
            $('Field' + columnId + '-1').value + $('Field' + columnId + '-2').value;
        return phone;
    },
...
var RunningTotal = Class.create({
    ...
    organizeMerchantFields: function (rawMerchantFields) {
        ...
        for (var i = 0; i < rawMerchantFields.length; i++) {
            ...
            if (field.Typeof == 'checkbox') {
                ...
                var textToIndex = this.getTextToIndexs(field);
                for (var key in field.Choices) {
                    if (typeof (field.Choices[key]) == 'object') {
                        var index = textToIndex[field.Choices[key].Choice];
    ...
    getTextToIndexs: function (field) {
        var count = 0;
        var textToIndex = new Array();
        for (var key in field.Choices) {
            var el = $('Field' + field.ColumnId + '_' + count);
            if (el) {
                textToIndex[el.value] = count;
            }
            count += 1;
        }
        return textToIndex;
    ...
    getElementValue: function (field, el) {
        if (field.Typeof == 'checkbox' || field.Typeof == 'radio') {
            ...
            $(field.ColumnId).value = $(field.ColumnId).value.replace(/[^0-9]/g, '');
            $(field.ColumnId + '-1').value = $(field.ColumnId + '-1').value.replace(/[^0-9]/g, '');
            var integer = $(field.ColumnId).value;
...
var PublicForm = Class.create({
    ...
    addOffsetForCaptcha: function () {
        ...
        if ($$('.captcha').length > 0 && IE6) {
            this.formLogic.offset = 30;
        }
    },
    initCalendars: function () {
        var dateLIs = $$('.date', '.eurodate');
        for (var i = 0; i < dateLIs.length; i++) {
            ...
            var onSelectFunction = (dateLIs[i].className.indexOf('eurodate') > -1) ? selectEuroDateOnForm : selectDateOnForm;
            Calendar.setup({
                inputField: "Field" + calendaFieldID,
                displayArea: "cal" + calendaFieldID,
                button: "pick" + calendaFieldID,
                ifFormat: "%B %e, %Y",
...
Object.observeEvent(window, 'message', resizeCallFromParent);
...
function resizeCallFromParent(event) {
    if (__PF.formLogic) {
        __PF.formLogic.resizeCallFromParent(event);
    }
}

function ratingToggle() {
    __PF.fieldLogic.ratingToggle();
}