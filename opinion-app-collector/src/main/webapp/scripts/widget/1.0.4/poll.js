									...
									if(u.options.list.length != 0) {
			                        	...
			                        	for (var i = 0; i < u.options.list.length; i++) {
			                        		if(u.options.list[i].optionKindId == 0) {
			                            		var k = $("<li class=\"choice\"><label><div class=\"choice-image\" style=\"overflow: hidden;\"></div><div class=\"choice-text\"><span><input type=\"radio\" class=\"radio-option\" id=\"" + u.options.list[i].optionId + "\" name=\"" + F + "\" optionindex=\"" + i + "\" optiontext=\"" + u.options.list[i].text + "\" optionkindid=\"" + u.options.list[i].optionKindId + "\" value=\"" + u.options.list[i].value + "\" autocomplete=\"off\" /></span><strong>" + u.options.list[i].text + "</strong></div></label></li>");
	...
	function init() {
		...
		*/
		...
		$(document).ready(function($) {
			...
			var target = /(^https?:\/\/[^\/]*(\:\d+)?)?\/.*scripts\/widget\/1.0.4\/poll\.js/; 
