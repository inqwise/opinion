		...
		(function ($) {
			...
			function init() {
				...
				var getDetails = function(params) {
					...
					//console.log("get time stamp -> " + $.fn.jquery + "_______");
				...
				function j(t) {
			        ...
			        */
					...
					_el.find('li.control .label-control-number').each(function(i, el) {
			        	//console.log(survey.controls.startNumerator + i);
						$(el).html((survey.controls.startNumerator + i) + ".&nbsp;");
					});
					...
					if(data.sheets.list.length > 0) {
						...
						$("<div>" +
							...
							"<div class=\"theme-page-actions ui-page-actions-a\" data-theme=\"a\" data-form=\"ui-page-actions-a\">" +
								...
								"</div>" +
								"<div class=\"container-paging\" style=\"" + (data.showPaging != undefined && data.showPaging == false ? "display: none" : "") + "\">Page <b class=\"label-start-page-number\">1</b> of <b class=\"label-end-page-number\">...</b></div>" +
							"</div>" +
				...
				*/
				                    ...
				                    for (var x = 0; x < survey.controls.list.length; x++) {
				                        if (survey.controls.list[x].controlId == F) {
				                        	*/
				                            ...
				                            if (u.isEnableNote) {
				                            	if(u.note != null) {
				                            		c.find('.label-control-note').html($.replaceURLWithHTMLLinks(u.note.replace(/\n/gi, "<br/>").replace(/%0A/gi, "<br/>"), true)).show();
				                            	}
				                            }
				        			...
				        			if(u.key != undefined) {
				        				...
				        				if (_data.guidTypeId == 1 && _data.modeId == 1) {
		                                    ...
		                                    if(u.answerValue != undefined) {
		                                    	...
		                                    	// set + .trigger('change')
		                                    	// from params
		                                    	if(_data.params != undefined) {
		    		        						if(_data.params[u.key] != undefined) {
		    		        							f.val(_data.params[u.key]).trigger('change');
		    		        						}
		                                    	}
				...
				$(document).ready(function($) {
					...
					var target = /(^https?:\/\/[^\/]*(\:\d+)?)?\/.*scripts\/widget\/1.0.5\/survey\.js/; 
					findScript(target).each(function() {
						...
						if(config.guid != undefined) {
							...
							fromConfig(config, function(config, data) {
								// do render
								render(el, config, data, isBody);
							});
						
						} else {
							
							if(inqwise.surveys != undefined) {
								for(var i = 0; i < inqwise.surveys.length; i++) {
									
									var config = inqwise.surveys[i];
									if(config.guid != undefined) {
				...
				inqwise = {
					...
					load : function(index) {
						...
						//console.log("from survey ->> " + $.fn.jquery + "_________" + $.getTimestamp());
						
						if(index != undefined) {
							if(inqwise.surveys[index] != undefined) {
								// load
								var config = inqwise.surveys[index];
								if(config.guid != undefined) {
	...
	if (window.jQuery === undefined || window.jQuery.fn.jquery !== '1.10.2') {
		loadScript("//ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js", function () {
			...
			(function($) {
				...
				console.log("from survey ->> " + $.fn.jquery);
		...
		console.log("dont load jquery ->> " + jQuery.fn.jquery);
